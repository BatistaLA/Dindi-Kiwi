...
onready var animacaoPorta = $PortaMadeira/AnimationPlayer
...
func abrirPorta():
	if entrada == true and portaAberta == false:
		if nomeEntrou == "PlayerDindi" and Global.dindiChaveAmarela > 0:
			if Input.is_action_just_pressed("selecionar_dindi"):
				Global.dindiChaveAmarela -= 1
				animacaoPorta.play("Abrir")
				portaAberta = true
				print(animacaoPorta.current_animation_position)
				Global.portaDeTrocaDeFase = true
				
		if nomeEntrou == "PlayerKiwi" and Global.kiwiChaveAmarela > 0:

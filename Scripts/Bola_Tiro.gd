...
func _on_Bola_Tiro_body_entered(body):
	...
	if rocha == true:
		Global.somColisaoAmarelaRocha = true
		queue_free()
	if madeira == true:
		Global.somColisaoAmarelaMadeira = true
		queue_free()
	if vidro == true:
		Global.somColisaoAmarelaVidro = true
		queue_free()
	if metal == true:
		Global.somColisaoAmarelaMetal = true
		queue_free()
	...
	if body is KinematicBody: 
		Global.somColisaoAmarelaCorpo = true # Quando bate em um corpo
		queue_free()
	...
	if get_tree().get_nodes_in_group("inimigo"):
		...
		Global.colidioAlvo = body.transform.origin
		Global.nomeColisaoBola = body.name
